...
- 未显式指定模式时，自动检查先尝试主力浏览器；若主力浏览器已可被 Agent 立即使用，则优先主力浏览器。若主力浏览器仍卡在远程调试授权/连接阶段，再自动回落到专用浏览器。
...
```bash
node ./scripts/check-deps.mjs                                  # 自动选择：先试 primary；若 primary 不能立刻被 Agent 使用，再回落到 dedicated
...
自动模式会先试主力浏览器；但下列任务即使当前默认在 dedicated 上，也应显式 `--browser primary`，避免先在错误会话里兜一圈。典型触发：
...
**在用文章内容（总结、提取、引用、回答）之前，先确认拿到的是全文，而不是被付费墙/计量墙截断的残篇。** 这是「目标内容拿到了吗？」在阅读类任务里的延伸——「页面能打开」「拿到几段文字」都不等于拿到全文。

判断是否被截断的信号：

- 正文明显偏短：只有开头几段就断，后面接订阅/登录召唤，而非完整文章。
- 文中插入付费/登录拦截：`Subscribe to keep reading`、`Sign in to read`、`Continue reading`、`成为付费会员`、`订阅以继续`、会员墙容器/遮罩等，把正文拦腰截断。
- 站点经验文件里记录的该站付费墙标记（见 `references/site-patterns/{domain}.md`）。

注意计量型付费墙（如 The Verge）：同一 URL 在配额内能读全文、配额耗尽后才被墙，所以同一链接前后表现可能不同，登录也未必能解锁；不要因为「之前读到过」就假定这次也是全文。

确认被截断、且无法拿到全文时，**绝不把残篇当全文使用**：

- 自动/批量任务（如 morning-report）：跳过该来源，换一篇可读的免费文章，并在结果里标注「该来源付费/被墙，已跳过」，不要硬总结截断内容。
- 交互任务：告诉用户该文付费/被墙、当前只能拿到部分内容，询问是否要用其他来源或由用户登录后继续。
